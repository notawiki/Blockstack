  ...
  <div>
    ...
    <div class="row" v-if="settings.tabs">
      ...
      <div
        ...
        v-for="(tab, index) in filterdTabs"
        :key="index"
    ...
    <div class="row" v-if="settings.archived">
      ...
      <div
        class="col-12 cursor-pointer"
        v-for="(tab, index) in filterdArchive"
        :key="index"
        @click="restoreArchivedTab(tab)"
        @keydown.enter="restoreArchivedTab(tab)"
        :tabindex="1100 + index"
      >
        {{ tab.name }}
      </div>
...
export default {
  ...
  computed: {
    filterdTabs () {
      ...
      for (let i in this.tabs) {
        ...
        if (this.applySearch(tab)) tabs.push(tab)
      }
    ...
    filterdArchive () {
      ...
      for (let i in this.archive) {
        if (tabs.length >= 10) break

        let tab = this.archive[i]
  ...
  methods: {
    applySearch (tab) {
      let low = string => string.toLowerCase()
    ...
    restoreArchivedTab (tab) {
      let index = this.archive.indexOf(tab)
